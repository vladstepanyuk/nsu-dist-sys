...
type requestContexts = map[string]chan WorkDone
...
func (tr *TasksRegistryActor) CreateCrackHashTask(ctx context.Context, hash string,
	wordRange common.PartRange) (chan WorkDone, chan string) {
	chToPutRes := make(chan WorkDone, 1)
	chToPutReqId := make(chan string, 1)
...
func (tr *TasksRegistryActor) processRequest(req taskReq) {
	...
	tr.pendingTasks[taskId] = &taskContext{
		cancel: cancel,
		requestsForTask: map[string]chan WorkDone{
...
func (tr *TasksRegistryActor) processCancelRequest(cancelReq cancelTaskReq) {
	...
	chToPutRes <- WorkDone{
		taskId.hash,
		taskId.wordRange,
		CrackResult{ResultType: RESULT_TYPE_CANCELED},
	}
